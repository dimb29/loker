<div class="flex flex-row ml-4" x-data="{showchat:false}">
    @if($openChatBox)
    @if($this->isAll)
        <div class="hidden flex-col w-2/6 h-auto">
    @else
        <div class="flex flex-col w-full h-auto">
    @endif
            <div class="flex sm:items-center rounded-t-lg justify-between py-3 border-b-2 border-gray-200 bg-blue-600 cursor-pointer p-4 h-16">
                <div class="w-full" @click="showchat = ! showchat">
                    <i id="angle1" class="fa-solid fa-angle-up hover:bg-blue-700 p-2 px-3.5 rounded-full rounded-full text-white cursor-pointer my-auto rotate"></i>
                </div>
                <div class="mr-1">
                    <i id="angle1" class="fa-solid fa-xmark hover:bg-blue-700 p-2 px-3.5 rounded-full text-white cursor-pointer my-auto" wire:click="closeChatBox"></i>
                </div>
            </div>
            <div id="dropdown1" class="bg-white h-96 overflow-hidden overflow-y-auto border shadow-lg" wire:poll.visible x-show="showchat"
                ...
                @if($chat_list->chat_to_user != null && $chat_list->chat_to_user->id != $this->usid)
                    @if($chat_list->to == $this->usid || $chat_list->from == $this->usid)
                        <div wire:click="openMobileChat([{{$chat_list->to}},'{{$chat_list->type_to}}',{{$chat_list->id}}])" class="flex flex-row w-full border-b-2 border-gray-200 hover:bg-gray-200 p-4 cursor-pointer">
                            ...
                            <div class="flex flex-col ml-2 h-12">
                                <p class="text-sm font-bold truncate pl-1 textllll">{{$chat_list->chat_to_user->first_name.' '. $chat_list->chat_to_user->last_name}}</p>
                                @foreach($singlechat as $schat)
                                    ...
                                    <div class="flex flex-row text-sm text-gray-600 -mt-1 pl-1 w-full">
                                        <p class="break-all truncate w-72 md:w-36 lg:w-56">{!! Str::limit($singchat->desc, 66) !!}</p>
                ...
                @elseif($chat_list->chat_to_employer != null && $chat_list->chat_to_employer->id != $this->usid)
                    @if($chat_list->to == $this->usid || $chat_list->from == $this->usid)
                    <div wire:click="openMobileChat([{{$chat_list->to}},'{{$chat_list->type_to}}'])" class="flex flex-row w-full border-b-2 border-gray-200 hover:bg-gray-200 p-4 cursor-pointer">
                        ...
                        @endif
                        <div class="flex flex-col ml-2 h-12">
                            <p class="text-sm font-bold truncate pl-1 textllll">{{$chat_list->chat_to_employer->name}}</p>
                            @foreach($singlechat as $schat)
                                @php 
                                $cfrom = $chat_list->from; $this->cfrom = $cfrom;
                                $ctype = $chat_list->type; $this->ctype = $ctype;
                                ...
                                <!-- hidden md:hidden lg: -->
                                <div class="flex flex-row text-sm text-gray-600 -mt-1 pl-1 w-full">
                                    <p class="break-all truncate w-72 md:w-36 lg:w-56">{!! Str::limit($singchat2->desc, 66) !!}</p>
                                    @if($countchat->sum('read') != 0)
                                    <p class="text-white text-xs bg-green-400 rounded-2xl my-auto px-2 mr-0 right-0">{{$countchat->sum('read')}}</p>
                                    @endif
                                </div>
        ...
        @if($this->isAll)
        <div class="flex-1 justify-between flex flex-col rounded-t-lg" x-data="{SendMessage:false}">
        @else
        <div class="hidden sm:flex-1 justify-between flex flex-col rounded-t-lg" x-data="{SendMessage:false}">
        @endif
            @if($chatOpen)
            <div class="flex sm:items-center justify-between py-3 border-b-2 border-gray-200 bg-blue-600 p-4 h-16 rounded-t-lg">
                <div class="relative flex items-center space-x-4">
                    @if($totype == 'employer')
                        @if($this->isAll)
                        <i class="fa-solid fa-arrow-left text-white cursor-pointer hover:bg-blue-800 rounded-xl p-2 px-3.5" wire:click="closeChat"></i>
                        @else
                        <i class="fa-solid fa-arrow-left text-white cursor-pointer hover:bg-blue-800 rounded-xl p-2 px-3.5" wire:click="closeMobileChat"></i>
                        @endif
                        <div class="relative">
                        ...
                        <div class="flex flex-col leading-tight">
                            ...
                            @else
                            <span class="text-white mr-3" style="height: 30px">{{$headchat->name}}</span>
                            @endif
                            </div>
                            <span class="text-sm text-white -mt-1">{{$totype}}</span>
                        </div>
                    @elseif($totype == 'user')
                        @if($this->isAll)
                        <i class="fa-solid fa-arrow-left text-white cursor-pointer hover:bg-blue-800 rounded-xl p-2 px-3.5" wire:click="closeChat"></i>
                        ...
                        <div class="flex flex-col leading-tight">
                            ...
                            @else
                            <span class="text-white mr-3" style="height: 30px">{{$headchat->first_name .' '. $headchat->last_name}}</span>
                            @endif
                            </div>
                            <span class="text-sm text-white -mt-1">{{$totype}}</span>
                        </div>
            ...
            <div class="bg-white h-96 border shadow-lg">
                <div id="messages"
                class="flex flex-col h-10/12 space-y-4 p-3 overflow-y-auto scrollbar-thumb-blue scrollbar-thumb-rounded scrollbar-track-blue-lighter scrollbar-w-2 scrolling-touch">
                    @foreach($chats as $chatkeys => $chat)
                        @if(Auth::user()->id == $chats[$chatkeys]['from'])
                            <div class="chat-message" wire:poll.visible>
                ...
                <div class="border-t-2 border-gray-200 px-4 pt-2 mb-2">
