...
<div class="py-4">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="border-b-2 border-solid my-4"><p class="text-lg font-bold">Kelas Anda</p></div>
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-4">
            ...
            @if ($isOpen)
                @include('livewire.on-class.create')
            @endif
                    ...
                    <div class="max-w-full sm:max-w-sm rounded overflow-hidden shadow-lg">
                        <div class="px-6 py-4 text-more__container">
                            <div class="font-bold text-xl mb-2">
                                @php
                                $kelas_t = $kelas->classtitle;
                                for($i=0;$i < count($kelas_t);$i++){
                                    if($i+1 == count($kelas_t)){
                                        echo $kelas_t[$i]->title;
                                    }else{
                                        echo $kelas_t[$i]->title." - ";
                                    }
                                }
                        ...
                        <div class="px-6 pt-4 pb-2">
                            ...
                            <x-jet-delete-button id="{{$kelas->id}}" wire:click="delete({{$kelas->id}})" 
            ...
            @if($materiOpen)
                @include('livewire.on-class.materi')
            @endif
            @if($benefitOpen)
                @include('livewire.on-class.benefit')
            @endif
        </div>
        <div class="py-4">
            {{ $kelass->links() }}
        </div>
        <div class="border-b-2 border-solid my-4"><p class="text-lg font-bold w-auto">Kelas Yang Anda Beli</p></div>
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-4">
                ...
                @foreach($class as $kelas)
                    <div class="my-1 shadow-lg rounded cursor-pointer" onclick="window.location=`{{url('onclass/'.$kelas->id)}}`">
                        @foreach($kelas->images as $img)
                            @if($loop->first)
                                <img src="{{url($img->url)}}" alt="img-class" class="w-full">
                            @endif
                        @endforeach
