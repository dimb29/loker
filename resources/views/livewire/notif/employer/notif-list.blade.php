                        ...
                        <div class="grid grid-cols-6 gap-6 h-screen">
                            <div class="col-span-10 sm:col-span-6 h-screen">
                                    <div class="flex flex-row border-b pb-6">
                                        <div class="space-x-8 sm:-my-px sm:flex mr-1">
                                            <x-jet-nav-link href="{{ route('employer.notif') }}" :active="request()->routeIs('employer.notif')" class="font-semibold text-xl">
                                                {{ __('Notifikasi') }}
                                            </x-jet-nav-link>
                                        </div>
                                        <div class="space-x-8 sm:-my-px sm:flex ml-1">
                                            <x-jet-nav-link href="{{ route('employer.notif.star') }}" :active="request()->routeIs('employer.notif.star')" class="font-semibold text-xl">
                                                {{ __('Notifikasi Berbintang') }}
                                            </x-jet-nav-link>
                                        </div>
                                    </div>
                                @if($isOpen)
                                <div class="overflow-scroll hntf pt-2">
                                    <div class="flex flex-row">
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <p>
                                                Dari : 
                                                @if($notiv->notif_user->cv != null)
                                                <a href="{{url($notiv->notif_user->cv)}}" class="text-blue-500 underline" target="_blank">{{$notiv->notif_user->first_name . ' ' .$notiv->notif_user->last_name}}</a>
                                                @else
                                    ...
                                    <div class="relative cursor-pointer hover:bg-gray-300"
                                        ...
                                        @if($notif->notif_type == 4)
                                            <div class="flex flex-row px-1 py-2">
                                                <div class="flex flex-row w-full">
                                                    ...
                                                    @endif
                                                    <h1 class="truncate">{{$notif->notif_user->first_name .' '. $notif->notif_user->last_name}} mulai mengikuti anda.</h1>
                                                </div>
                                            </div>
                                        ...
                                        <div wire:click="clickOpen({{$notif->id}})" class="flex flex-row px-1 py-2">                                            
                                            <div class="flex flex-row w-1/4">
                                                ...
                                                <img src="{{url('storage/photos/red-circle.png')}}" alt="" class="w-2 h-2 -ml-2">
                                                @endif
                                                <h1 class="truncate font-semibold">{{$notif->title}}</h1>
                                            </div>
